...
# the number of each element in the molecule. This statement will import the 
# parse_formula function from the formula.py file into your chemistry.py 
from formula import parse_formula
                         
def main():
    #Get formula and mass from user
    formula = input("Enter first the chemical formula for a molecule: ")
    mass = float(input("Enter second the chemical sample in grams: "))

    # Call and store make_periodic_table
    periodic_table = make_periodic_table()

    # Call and store parse_formula
    compound_list = parse_formula(formula, periodic_table)

    # Call and store compute_molar_mass
    ...
    #STRETCH
    known_molecules = create_known_molecules_dict()
    formula_name = get_formula_name(formula, known_molecules)
...
def create_known_molecules_dict():
    """Creates a known molecules dictionary
    parameters: none
    return: dictionary
    """
...
def compute_molar_mass(symbol_quantity_list, periodic_table_dict):
    ...
    for inner_list in symbol_quantity_list:
        #Split symbol and quantity
        symbol = inner_list[0]
        quantity = inner_list[1]   
        
        #Computes total mass, by finding atomic mass in dictonary thanks to symbol and index, and then multiplying it by quantity. Finally it adds atomic mass to total mass
...
# This function computes the molar mass of the molecule of known compound.
